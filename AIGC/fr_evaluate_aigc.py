...
import pyiqa
import rapique  # Ensure rapique.py is in the same directory or in PYTHONPATH
import logging
...
FULL_REFERENCE_AIGC_LEGEND = r"""
Legend for AIGC Full-Reference Metrics:
...
3. RAPIQUE:
   - A metric that can operate with partial or full reference information, although sometimes used 
     in a reduced-reference or no-reference context. Here we demonstrate its full-reference usage.
   - Lower values typically indicate better quality or higher similarity (depending on the implementation).
   - Range: Implementation-specific. Often 0 to 100 or 0 to 1.
   - How It Works:
     RAPIQUE (Reduced-Reference and No-Reference) uses natural scene statistics and 
     perceptual features from deep networks to compare images. Here, we treat it 
     in a full-reference context as supported by rapique.py.
...
#     """
#     Extract the reference identifier from the test image name.
#     E.g.:
#       'A0001_00_00.jpg' -> returns 'A0001'
#       'B1234_01_02.png' -> returns 'B1234'
#     """
...
def evaluate_full_reference_aigc_metrics(test_dir, reference_dir, output_csv, possible_extensions=[".jpg", ".png", ".jpeg"]):
    """
    Evaluate full-reference AIGC metrics (LPIPS, DISTS, RAPIQUE) for all test images
    """
    ...
    try:
        rapique_metric = rapique.RAPIQUE(device=device)
        logging.info("Initialized RAPIQUE metric.")
    except Exception as e:
        logging.error(f"Failed to initialize RAPIQUE metric: {e}")
        rapique_metric = None

    try:
        with open(output_csv, mode='w', newline='', encoding='utf-8') as file:
            writer = csv.writer(file)
            header = ["Test Image", "Reference Image", "LPIPS", "DISTS", "RAPIQUE", "Evaluation Time (s)"]
            ...
            for test_image_name in os.listdir(test_dir):
                ...
                try:
                    ...
                    # Compute RAPIQUE
                    rapique_val = "N/A"
                    if rapique_metric is not None:
                        try:
                            rapique_val = rapique_metric.compute_quality_score(test_image, reference_image)
                        except Exception as e:
                            logging.error(f"Error computing RAPIQUE for {test_image_name}: {e}")
                            rapique_val = "Error"
